	...
	public function field_render_html($params = array()) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='form-group'>
				...
				<div class='row'>
					<div class='col-md-6'>
						<select data-placeholder='Choose a country' style='width: 100%' class='select2element' id='<?php echo $params['id']?>_selector_country' name='submitform[<?php echo $params['id'] ?>][country]' <?php echo required($params) ?>>
							<option value='none'>Choose a country</option>
							<?php
					...
					<div class='col-md-6'>
						<select data-placeholder='Choose a state' style='width: 100%' class='select2element' id='<?php echo $params['id'] ?>_selector_state' name='submitform[<?php echo $params['id'] ?>][state]' <?php required($params) ?>>
						<option value='none'>Choose a state</option>
						</select>
					</div>
				</div>
